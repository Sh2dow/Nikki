...
namespace Nikki.Support.Carbon.Framework
{
    internal class DatabaseLoader : IDisposable
    {
        ...
        public void ExportDatabase(BinaryWriter bw, bool serialized = true)
        {
            try
            {
                // Export each block managed by the loader
                _db.CarTypeInfos.Assemble(bw, _options.Watermark);
                _db.Collisions.Assemble(bw, _options.Watermark);
                _db.Materials.Assemble(bw, _options.Watermark);
                _db.TPKBlocks.Assemble(bw, _options.Watermark);
                _db.SunInfos.Assemble(bw, _options.Watermark);
                // Add other blocks as needed...
            }
            catch (Exception e)
            {
                Console.WriteLine($"Error while exporting: {e.Message}");
            }
        }
