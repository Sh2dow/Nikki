...
namespace Nikki.Support.Carbon.Framework
{
    ...
    public class CarTypeInfoManager : Manager<CarTypeInfo>
    {
        ...
        public override void Export(string cname, BinaryWriter bw, bool serialized = true)
        {
            ...
            if (index == -1)
            {
                // Export all collections if specific cname does not exist
                try
                {
                    foreach (var carTypeInfo in this)
                    {
                        if (serialized)
                            carTypeInfo.Serialize(bw);
                        else
                            carTypeInfo.Assemble(bw);
                    }
                }
                catch (Exception e)
                {
            ...
            {
                // Export the specific collection
