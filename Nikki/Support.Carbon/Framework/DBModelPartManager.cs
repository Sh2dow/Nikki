...
namespace Nikki.Support.Carbon.Framework
{
    ...
    public class DBModelPartManager : Manager<DBModelPart>
    {
                ...
                foreach (Parts.CarParts.RealCarPart realpart in model.ModelCarParts)
                {
                    ...
                    else
                    {
                        ...
                        if (original != offset)
                        {
                            ...
                            logger.WriteLine(
                                $"Comparing keys 9x{original.GetHashCode():X8} vs 0x{offset.GetHashCode():X8}");
                            ...
                            throw new Exception(
                                "Internal error has occurred: DBMP:AO Key Exception. Please report to the developer immediately, " +
        ...
        private CPAttribute[] ReadAttribs(BinaryReader br, BinaryReader str, int maxlen,
            Dictionary<uint, CustomCP> cpmap)
        {
            var size = br.ReadInt32();
            var offset = br.BaseStream.Position;
        ...
        public override void Export(string cname, BinaryWriter bw, bool serialized = true)
        {
            ...
            if (index == -1)
            {
                // Export all collections if the collection with the given cname does not exist
                try
                {
                    foreach (var dbModelPart in this)
                    {
                        if (serialized)
                            dbModelPart.Serialize(bw);
                        else
                            dbModelPart.Assemble(bw);
                    }
                }
                catch (Exception e)
                {
