            ...
            else
            {
                if (header.Game != this.GameINT)
                {
                    try
                    {
                        // Attempt conversion
                        collection.Disassemble(br);
                        Console.WriteLine($"Converting collection from {header.Game} to {this.GameINT}");
                        // Apply transformation logic if needed
                        this.ConvertCollectionToGame(collection, header.Game, this.GameINT);
                    }
                    catch (Exception ex)
                    {
                        throw new Exception(
                            $"Failed to convert collection from {header.Game} to {this.GameINT}: {ex.Message}");
                    }
                }
